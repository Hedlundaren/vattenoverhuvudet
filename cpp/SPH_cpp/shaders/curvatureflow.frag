...
void main() {
	...
	if(particleDepth == 0.0f) {
		...
		//vDepth
		//const float dt = 0.000045f;
		//const float dzt = 2.0f;
		//vDepth+dist
		...
		//outDepth =  dxyz.z * dt * (1.0f + (abs(dxyz.x) + abs(dxyz.y)) * dzt);
		//outDepth = (abs(dxyz.x) + abs(dxyz.y)) * dzt;
