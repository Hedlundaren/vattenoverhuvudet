...
__kernel void taskParallelCalculateDensity(__global float* densities,
    ...
    for(int i = 0; i < howMany; i++){
        ...
        if( radius < kernelSize){
            densities[id] = densities[id] + mass*(315/(64*pi*h^9)) * (kernelSize^2 - radius^2)^3;
        }
    }
}
...
__kernel void taskParallelCalculatePressureForce(__global float3* pressureForces,
    ...
    for(int i = 0; i < howMany; i++){
        ...
        if( radius < kernelSize){
            pressureForces[id].x = pressureForces[id].x + mass * ( (p_i + p_id)/(2*density[i]) ) * (15 / (pi * kernelSize^6)) * 3 * (kernelSize - radius)^2 * (- r[id].x / radius);
            pressureForces[id].y = pressureForces[id].y + mass * ( (p_i + p_id)/(2*density[i]) ) * (15 / (pi * kernelSize^6)) * 3 * (kernelSize - radius)^2 * (- r[id].y / radius);
            pressureForces[id].z = pressureForces[id].z + mass * ( (p_i + p_id)/(2*density[i]) ) * (15 / (pi * kernelSize^6)) * 3 * (kernelSize - radius)^2 * (- r[id].z / radius);
        }
    }
}
...
__kernel void taskParallelCalculateViscosityForce(__global float3* viscosityForces,
    ...
    for(int i = 0; i < howMany; i++){
        ...
        if( radius < kernelSize ){
            viscosityForces[id].x = viscosityForces[id].x + mass * ( (velocities[i].x - velocities[id].x) / densities[i] ) * (45 / (pi * kernelSize^6)) * (kernelSize - radius);
            viscosityForces[id].y = viscosityForces[id].y + mass * ( (velocities[i].y - velocities[id].y) / densities[i] ) * (45 / (pi * kernelSize^6)) * (kernelSize - radius);
            viscosityForces[id].z = viscosityForces[id].z + mass * ( (velocities[i].z - velocities[id].z) / densities[i] ) * (45 / (pi * kernelSize^6)) * (kernelSize - radius);
        }
    }
}
...
__kernel void taskParallelCalculateTensionForce(__global float3* tensionForces,
    ...
    for(int i = 0; i < howMany; i++){
        ...
        if( radius < kernelSize ){
            cs = cs + mass * (1 / densities[i]) * (315/(64*pi*h^9)) * (kernelSize^2 - radius^2)^3;

            n.x = n + mass * (1 / densities[i]) * (-315/(64*pi*kernelSize^9)) * 6 * (kernelSize^2 - radius^2)^2 * r.x;
            n.y = n + mass * (1 / densities[i]) * (-315/(64*pi*kernelSize^9)) * 6 * (kernelSize^2 - radius^2)^2 * r.y;
            n.z = n + mass * (1 / densities[i]) * (-315/(64*pi*kernelSize^9)) * 6 * (kernelSize^2 - radius^2)^2 * r.z;

            laplacianCs = laplacianCs + mass * (1 / densities[i]) * (315/(64*pi*kernelSize^9)) * (24 * radius^2 * (kernelSize^2 - radius^2) - 6 * (kernelSize^2 - radius^2)^2);
